...
/*   Updated: 2024/11/26 16:26:00 by mlaffita         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int process_line(char *buffer, char **line)
{
    char *n_buff;
...
int append_remaining_buffer(char *buffer, char **line)
{
    *line = ft_strjoin(*line, buffer);
    if (!*line)
        return (0);
...
char *get_next_line(int fd)
{
    ...
    while (1)
    {
        ...
        if (!handle_end_of_file(rd, buffer, &line))
            return (ft_free(&line));
        if (process_line(buffer, &line))
            return (line);
        if (!append_remaining_buffer(buffer, &line))
            return (ft_free(&line));
        if (rd == GNL_EOF)
            break ;
    }
